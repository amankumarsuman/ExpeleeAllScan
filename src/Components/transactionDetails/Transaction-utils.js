async function getTransactionDetails(transactionHash, network) {
    ...
    console.log(network,transactionHash,"check")
    let baseUrl, apiKey;
...
async function getWalletData(network, walletAddress) {
    ...
    const tokenBalanceOfPolygon = formatAmount(tokenDataOfPolygon.result, decimals, symbol);
...
export {getTransactionDetails,formatTransactionDetails,getContractOrTokenDetails,getWalletData}